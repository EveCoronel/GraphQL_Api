...
const { getAge, sendEmail } = require("../utils/utils");
const passport = require("passport");
const multer = require("multer");
const envConfig = require("../config");
const usersDao = require("../models/dao/users/usersDao.mongo");
...
class AuthControllers {
      ...
      req.session.destroy((err) => {
        if (!err) {
          res.render(
            path.join(
              process.cwd(),
              "Public/views/pages/authentication/logout.ejs"
            ),
            { email: user.email }
          );
        } else {
  ...
  async register(req, res, next) {
    let { username, password, address, birthDate, countryCode, phoneNumber } =
      req.body;
    let cart = await cartsDao.createCart();
    let profilePicture = `Avatar${Math.floor(Math.random() * 12) + 1}.png`;
    if (req.file)
      profilePicture = `${username}.${req.file.originalname.split(".")[1]}`;
    let newUser = {
      email: username,
